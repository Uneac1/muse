...
export class AgentProfileMemoryModel {
  list(): AgentProfileMemory[] {
    return db.prepare('SELECT * FROM agent_profile_memory ORDER BY confidence DESC, updated_at DESC, id DESC').all() as AgentProfileMemory[];
  }

  getByKey(key: string): AgentProfileMemory | undefined {
    return db.prepare('SELECT * FROM agent_profile_memory WHERE key = ?').get(key) as AgentProfileMemory | undefined;
  }
  ...
  upsert(data: {
    ...
    `).run(
      data.key,
      data.value,
      data.category || current?.category || 'preference',
      confidence,
      data.source || current?.source || 'manual',
      data.lastObservedAt || current?.last_observed_at || new Date().toISOString()
    );
    return this.getByKey(data.key)!;
  }
}

export class AgentSkillJournalModel {
  private toView(row: AgentSkillJournal): AgentSkillJournalView {
    return {
      ...row,
  ...
  list(limit = 20): AgentSkillJournalView[] {
    return db
      .prepare('SELECT * FROM agent_skill_journal ORDER BY score DESC, updated_at DESC, id DESC LIMIT ?')
      .all(limit)
      .map((row) => this.toView(row as AgentSkillJournal));
  }

  findByPattern(pattern: string): AgentSkillJournal | undefined {
    return db.prepare('SELECT * FROM agent_skill_journal WHERE pattern = ?').get(pattern) as AgentSkillJournal | undefined;
  }
  ...
  remember(data: {
    ...
    const normalizeEvidence = (value: string) => {
      ...
      if (!text) return '';
      if (!text.startsWith('[Muse Section Context]')) return text;
      const taskSplit = text.split(/\[Task\]/i);
      if (taskSplit.length > 1) return taskSplit.slice(1).join('[Task]').trim();
      const userSplit = text.split(/\b(?:user|message)\s*:/i);
      if (userSplit.length > 1) return userSplit.slice(1).join(' ').trim();
      return '';
    };
    const nextEvidence = [...new Set([...(current ? parseJson<string[]>(current.evidence_json, []) : []), ...(data.evidence || [])])]
...
export class AgentAutonomyConfigModel {
  get(): AgentAutonomyConfig {
    const row = db.prepare('SELECT * FROM agent_autonomy_config WHERE id = 1').get() as any;
    if (!row) {
  ...
  update(input: Partial<AgentAutonomyConfig>): AgentAutonomyConfig {
    ...
    `).run(
      ...
      (input.memoryIngestionEnabled ?? current.memoryIngestionEnabled) ? 1 : 0,
      (input.ruleAutomationEnabled ?? current.ruleAutomationEnabled) ? 1 : 0,
      (input.profileLearningEnabled ?? current.profileLearningEnabled) ? 1 : 0,
      (input.skillLearningEnabled ?? current.skillLearningEnabled) ? 1 : 0,
      (input.backupEnabled ?? current.backupEnabled) ? 1 : 0,
      input.backupDir ?? current.backupDir,
      ...
      input.lastCompactedAt ?? current.lastCompactedAt
    );
    return this.get();
  }
}

export class AgentAutonomyRunModel {
  list(limit = 10): AgentAutonomyRun[] {
    return db.prepare('SELECT * FROM agent_autonomy_runs ORDER BY started_at DESC, id DESC LIMIT ?').all(limit).map((row: any) => ({
      id: row.id,
      status: row.status,
      summary: row.summary || '',
  ...
  create(): number {
    const result = db.prepare('INSERT INTO agent_autonomy_runs (status) VALUES (?)').run('success');
...
export class AgentRuntimeSnapshotModel {
  list(limit = 10): AgentRuntimeSnapshot[] {
    return db.prepare('SELECT * FROM agent_runtime_snapshots ORDER BY created_at DESC, id DESC LIMIT ?').all(limit).map((row: any) => ({
      id: row.id,
      status: row.status,
      summary: row.summary || '',
      ...
      recoveries: parseJson<string[]>(row.recoveries_json, []),
      metadata: parseJson<Record<string, any>>(row.metadata_json, {}),
      createdAt: row.created_at,
    }));
  }

  create(payload: Omit<AgentRuntimeSnapshot, 'id' | 'createdAt'>) {
    db.prepare(`
    ...
    `).run(
      payload.status,
      payload.summary || '',
      payload.aiHealthyCount || 0,
      payload.aiTotalCount || 0,
      payload.proxyHealthy ? 1 : 0,
      payload.proxyMode || 'unknown',
      payload.memoryHealthy ? 1 : 0,
      payload.ruleHealthy ? 1 : 0,
      payload.newspaperHealthy ? 1 : 0,
      payload.systemLoad || 0,
      payload.memoryUsagePct || 0,
      JSON.stringify(payload.anomalies || []),
      JSON.stringify(payload.recoveries || []),
...
export class AgentRecoveryIncidentModel {
  list(limit = 20): AgentRecoveryIncident[] {
    return db.prepare('SELECT * FROM agent_recovery_incidents ORDER BY detected_at DESC, id DESC LIMIT ?').all(limit).map((row: any) => ({
      id: row.id,
  ...
  getByFingerprint(fingerprint: string): AgentRecoveryIncident | undefined {
    const row = db.prepare('SELECT * FROM agent_recovery_incidents WHERE fingerprint = ?').get(fingerprint) as any;
  ...
  upsert(payload: {
    ...
    `).run(
      ...
      payload.detail,
      payload.fingerprint,
      payload.recoveryAction || current?.recoveryAction || '',
      payload.recoveryResult || current?.recoveryResult || '',
      JSON.stringify(payload.metadata || current?.metadata || {}),
      payload.resolvedAt ?? current?.resolvedAt ?? null
    );
    return this.getByFingerprint(payload.fingerprint)!;
  }
}

export class AgentCapabilityWeightModel {
  list(limit = 20): AgentCapabilityWeight[] {
    return db.prepare('SELECT * FROM agent_capability_weights ORDER BY weight DESC, updated_at DESC, id DESC LIMIT ?').all(limit).map((row: any) => ({
      id: row.id,
  ...
  remember(payload: {
    ...
    `).run(
      ...
      payload.outcome,
      payload.summary || '',
      payload.source || current?.source || 'autonomy'
    );
    return this.getByCapability(payload.capability)!;
  }
}

export class AgentRuntimeEventModel {
  private toView(row: any): AgentRuntimeEvent {
    return {
      id: Number(row.id),
      layer: row.layer || 'raw',
      ...
      title: row.title || '',
      detail: row.detail || '',
      content: parseJson<Record<string, any>>(row.content_json, {}),
      confidence: Number(row.confidence || 0.5),
      shared: !!row.shared,
      status: row.status || 'active',
      originRefs: parseJson<string[]>(row.origin_refs_json, []),
  ...
  create(payload: {
    ...
    `).run(
      ...
      payload.eventType || 'observation',
      payload.title || '',
      payload.detail || '',
      JSON.stringify(payload.content || {}),
      Math.max(0.05, Math.min(1, Number(payload.confidence ?? 0.5))),
      payload.shared === false ? 0 : 1,
      payload.status || 'active',
      JSON.stringify(payload.originRefs || []),
      payload.compactedAt || null,
    );
