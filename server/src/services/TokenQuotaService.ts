...
const DEFAULT_ANALYTICS_URLS: Record<TokenProvider, string> = {
...
function extractCardNearLabel(text: string, key: string, label: string, tone: TokenQuotaCard['tone']): TokenQuotaCard | null {
...
function findQuotedSegments(text: string, candidates: string[]) {
  return candidates.filter((item) => text.includes(item));
}

function parseSessionPayload(sessionPayload: string, format: TokenSessionFormat) {
...
function extractUsageSections(text: string): TokenUsageSection[] {
  const sections: TokenUsageSection[] = [];
  const dateLabels = uniqueStrings(text.match(/\d{1,2}月\d{1,2}日/g) || []);
  const legends = uniqueStrings(
    findQuotedSegments(text, ['Desktop App', 'CLI', 'IDE Extension', '个人使用', '团队使用', 'ChatGPT', 'Claude Code'])
  );
...
function parseOpenAiAnalytics(html: string, text: string, sourceUrl: string, finalUrl: string): TokenAnalyticsSnapshot {
  ...
  ).map((item) => JSON.parse(item) as TokenQuotaCard);
...
function parseClaudeAnalytics(provider: TokenProvider, html: string, text: string, sourceUrl: string, finalUrl: string): TokenAnalyticsSnapshot {
  const cards = uniqueStrings(
    [
...
export class TokenQuotaService {
  getDefaultAnalyticsUrl(provider: TokenProvider) {
  ...
  async syncAccount(account: TokenAccount): Promise<TokenAnalyticsSnapshot> {
    ...
    if (account.provider === 'openai_codex') {
      const authMethod = (account.auth_method || 'session') as TokenAuthMethod;
      ...
      if ((authMethod === 'oauth' || authMethod === 'manual') && (account.refresh_token || account.access_token)) {
        ...
        return openAIAccountService.fetchWhamSnapshot(
          sourceUrl,
          tokenInfo.accessToken || account.access_token,
          account.external_account_id || tokenInfo.chatgptAccountId
        );
      }
    }
