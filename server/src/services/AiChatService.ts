...
export class AiChatService {
  private async getGeminiAuth(account: AiAccount) {
    if (account.auth_mode === 'google_oauth') {
      const clientId = account.oauth_client_id || config.adminGoogleClientId;
      const clientSecret = account.oauth_client_secret || config.adminGoogleClientSecret;
  ...
  async sendMessage(account: AiAccount, history: AiMessage[]): Promise<string> {
    if (account.provider === 'gemini') {
  ...
  private async sendOpenAiCompatible(account: AiAccount, history: AiMessage[]) {
    ...
    const response = await this.requestJson(
      joinUrl(account.base_url, '/chat/completions'),
      {
        'Content-Type': 'application/json',
  ...
  private async sendResponses(account: AiAccount, history: AiMessage[]) {
    const input: Array<{ role: 'system' | 'user' | 'assistant'; content: Array<{ type: 'input_text'; text: string }> }> = [];
    ...
    const response = await this.requestJson(
      joinUrl(account.base_url, '/responses'),
      {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${account.api_key}`,
      },
  ...
  private async sendAnthropic(account: AiAccount, history: AiMessage[]) {
    ...
    const response = await this.requestJson(
      joinUrl(account.base_url, '/v1/messages'),
      {
        'Content-Type': 'application/json',
  ...
  private async sendGemini(account: AiAccount, history: AiMessage[]) {
  ...
  private async powershellRequest(method: 'GET' | 'POST', url: string, headers: Record<string, string>, body: string) {
    const script = `
