...
import type { AiAccount, AiConnectionTestResult, AiMessage, AiRuntimeContext, MailMessage } from '../types';
...
type MuseToolServiceShape = {
  getSystemPrompt(): string;
  getToolNames(): string[];
  executeCalls(calls: MuseToolCall[], userMessage?: string): Promise<MuseToolExecution[]>;
  formatResultsForModel(results: MuseToolExecution[]): string;
  formatFallbackAnswer(results: MuseToolExecution[]): string;
};
...
let museToolService: MuseToolServiceShape | null = null;
const POWERSHELL_EXE = 'C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe';
const REQUEST_TIMEOUT_MS = 45000;

function getMuseToolService(): MuseToolServiceShape {
  if (!museToolService) {
    const { AiMuseToolService } = require('./AiMuseToolService') as { AiMuseToolService: new () => MuseToolServiceShape };
...
function getGeminiApiPrefix(baseUrl: string) {
...
function getGeminiModelsEndpoint(baseUrl: string) {
  const apiPrefix = getGeminiApiPrefix(baseUrl);
  return `${apiPrefix}/models`;
}

function getGeminiGenerateEndpoint(baseUrl: string, model: string) {
  const apiPrefix = getGeminiApiPrefix(baseUrl);
...
function isAnthropicProvider(provider: AiAccount['provider']) {
  return provider === 'claude' || provider === 'claude_code' || provider === 'anthropic_compatible';
}

function isResponsesProvider(provider: AiAccount['provider']) {
...
function getSystemPrompt(account: Pick<AiAccount, 'system_prompt'>) {
  return String(account.system_prompt || '').trim();
}

function buildIdentityPrompt(account: Pick<AiAccount, 'id' | 'provider' | 'name' | 'model' | 'base_url' | 'system_prompt'>) {
...
function buildEffectiveSystemPrompt(account: Pick<AiAccount, 'id' | 'provider' | 'name' | 'model' | 'base_url' | 'system_prompt'>) {
  return [buildIdentityPrompt(account), getSystemPrompt(account)].filter(Boolean).join('\n\n');
}

function buildResponseMeta(account: AiAccount): AiResponseMeta {
  return {
...
function buildInboxMailDigest(mails: MailMessage[]) {
  return mails
...
async function buildRuntimeContextPrompt(runtimeContext?: AiRuntimeContext | null) {
...
function inferProtocol(provider: AiAccount['provider'], model: string): AiProtocol {
...
function normalizeProtocolBaseUrl(protocol: AiProtocol, baseUrl: string) {
...
function getModelListEndpoint(account: AiAccount) {
...
function parseAnthropicText(json: any): string {
...
function parseResponsesText(json: any): string {
...
function buildNativeRuntimeToolDefinition(toolNames: string[]) {
  return {
    type: 'function',
...
function parseNativeToolPayload(raw: unknown): MuseToolCall | null {
  if (!raw) return null;
  if (typeof raw === 'object') {
    ...
    const topLevelArgs = Object.fromEntries(
      Object.entries(item).filter(([key]) => !['tool', 'args', 'arguments', 'parameters', 'input'].includes(key))
    );
    const mergedArgs = nestedArgsCandidates.reduce(
      (acc, current) => ({ ...acc, ...(current as Record<string, unknown>) }),
      { ...(topLevelArgs && typeof topLevelArgs === 'object' ? topLevelArgs : {}) }
    );
    return {
...
function parseResponsesNativeToolCalls(json: any): MuseToolCall[] {
...
function formatGeminiError(json: any, fallback: string): string {
  const error = json?.error;
  const details = Array.isArray(error?.details) ? error.details : [];
  const info = details.find((item: any) => item?.['@type'] === 'type.googleapis.com/google.rpc.ErrorInfo');
  const localized = details.find((item: any) => item?.['@type'] === 'type.googleapis.com/google.rpc.LocalizedMessage');
...
function formatGatewayError(json: any, fallback: string): string {
  const message =
    json?.error?.message ||
    json?.message ||
    json?.Message ||
...
function parseAvailableModels(raw: string) {
  try {
...
function rankGeminiModel(model: string) {
  ...
  if (normalized === 'gemini-2.5-flash') return 0;
  if (normalized === 'gemini-2.5-flash-lite') return 1;
  if (normalized === 'gemini-2.0-flash') return 2;
  if (normalized === 'gemini-2.0-flash-001') return 3;
  if (normalized === 'gemini-2.0-flash-lite') return 4;
  if (normalized === 'gemini-2.0-flash-lite-001') return 5;
  if (normalized === 'gemini-2.5-pro') return 6;
  return 100;
}

function getModelFamily(model: string) {
  const normalized = normalizeModelName(model);
  if (normalized.startsWith('models/')) return getModelFamily(normalized.replace(/^models\//, ''));
  if (normalized.startsWith('gemini')) return 'gemini';
  if (normalized.startsWith('claude') || normalized.includes('anthropic')) return 'claude';
  if (normalized.includes('codex')) return 'codex';
  if (/^gpt[-.0-9a-z]*/i.test(normalized)) return 'gpt';
  if (normalized.startsWith('deepseek')) return 'deepseek';
  if (normalized.startsWith('mimo')) return 'mimo';
  return 'other';
}

function buildModelCandidates(account: AiAccount) {
  ...
  if (account.provider === 'gemini' || String(account.name || '').toLowerCase().includes('gemini')) {
    return [...sameFamily, ...crossFamily].sort((left, right) => {
      ...
      if (normalizeModelName(left) === normalizeModelName(currentModel)) return -1;
      if (normalizeModelName(right) === normalizeModelName(currentModel)) return 1;
...
function findAnthropicFallbackModel(account: AiAccount, failedModel: string) {
  const availableModels = parseAvailableModels(account.available_models);
...
function findGeminiFallbackModel(account: AiAccount, failedModel: string) {
  const availableModels = parseAvailableModels(account.available_models)
...
function shouldRetryGeminiWithFallback(detail: string) {
...
export class AiChatService {
  private async getGeminiAuth(account: AiAccount) {
    if (account.auth_mode === 'google_oauth') {
      const clientId = account.oauth_client_id || config.googleClientId;
      const clientSecret = account.oauth_client_secret || config.googleClientSecret;
      if (!clientId || !clientSecret) {
        throw new Error('Gemini Google OAuth client is not configured');
      }
      const token = await oauthService.refreshGoogleToken(
        clientId,
        clientSecret,
    ...
    return {
      ...
      headers: {
        ...
        Authorization: `Bearer ${token.access_token}`,
          ...((account.oauth_project_id || config.googleProjectId) ? { 'x-goog-user-project': account.oauth_project_id || config.googleProjectId } : {}),
        },
      };
    }
  ...
  async testConnection(account: AiAccount): Promise<AiConnectionTestResult> {
  ...
  private async sendMessageOnce(account: AiAccount, history: AiMessage[]): Promise<{ content: string; responseMeta: AiResponseMeta }> {
    const protocol = inferProtocol(account.provider, account.model);
  ...
  private async sendNativeToolMessageOnce(account: AiAccount, history: AiMessage[]): Promise<NativeToolResult> {
    const protocol = inferProtocol(account.provider, account.model);
  ...
  async sendMessage(account: AiAccount, history: AiMessage[]): Promise<{ content: string; responseMeta: AiResponseMeta }> {
    ...
    for (const model of candidates) {
      ...
      try {
        ...
        if (model !== account.model) {
          logger.warn(`AI account ${account.id} succeeded with fallback model ${model} instead of ${account.model}`);
        }
        return result;
      } catch (error: any) {
  ...
  async sendNativeToolMessage(account: AiAccount, history: AiMessage[]): Promise<NativeToolResult> {
    const candidates = buildModelCandidates(account).slice(0, 16);
    const failures: string[] = [];
    const lockedFamily = getModelFamily(account.model);
  ...
  private toToolExecutionDetails(results: MuseToolExecution[]): MuseToolExecutionDetail[] {
  ...
  async sendModelSessionMessage(account: AiAccount, history: AiMessage[], runtimeContext?: AiRuntimeContext | null): Promise<{ content: string; actions: MuseUiAction[]; toolExecutions: number; toolDetails: MuseToolExecutionDetail[]; responseMeta: AiResponseMeta; degradedReason?: string; runtimeEventRefs: string[] }> {
    const museToolService = getMuseToolService();
    const runtimeContextPrompt = await buildRuntimeContextPrompt(runtimeContext);
    const implicitMemoryPrompt = agentAutonomyService.buildImplicitChatContext({
    ...
    const aggregatedResults: MuseToolExecution[] = [];
    let finalContent = '';
    let latestResponseMeta: AiResponseMeta | null = null;
    ...
    const actions = aggregatedResults.map((item) => item.uiAction).filter(Boolean) as MuseUiAction[];
    const runtimeEventRefs: string[] = [];
    if (runtimeContext?.path) {
      const event = agentAutonomyService.logEvent({
        layer: 'raw',
        scope: 'page',
  ...
  async sendMuseControlledMessage(account: AiAccount, history: AiMessage[], runtimeContext?: AiRuntimeContext | null) {
    return this.sendModelSessionMessage(account, history, runtimeContext);
  }

  private async sendOpenAiCompatible(account: AiAccount, history: AiMessage[], enableNativeTools = false): Promise<any> {
  ...
  private async sendResponses(account: AiAccount, history: AiMessage[], enableNativeTools = false): Promise<any> {
    ...
    if (!response.ok && response.transport !== 'powershell' && [400, 401, 403, 502].includes(response.status)) {
      logger.warn(`Responses endpoint failed through ${response.transport} for account ${account.id}, retrying PowerShell direct: ${raw.slice(0, 300) || response.status}`);
    ...
    if (!response.ok) {
      ...
      if (response.status === 400 || response.status === 404 || /not found|unsupported|unknown endpoint|invalid url/i.test(detail)) {
        logger.warn(`Responses endpoint failed for account ${account.id}, trying chat completions fallback: ${detail}`);
  ...
  private async sendAnthropic(account: AiAccount, history: AiMessage[], modelOverride?: string): Promise<{ content: string; responseMeta: AiResponseMeta }> {
    ...
    if (!response.ok) {
      ...
      if (!modelOverride && /1m\s*上下文|1m\s*context/i.test(detail)) {
        ...
        if (fallbackModel) {
          logger.warn(`Anthropic model ${model} rejected by upstream, retrying with fallback model ${fallbackModel}`);
  ...
  private async sendGemini(account: AiAccount, history: AiMessage[], modelOverride?: string): Promise<{ content: string; responseMeta: AiResponseMeta }> {
    const model = modelOverride || account.model;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (!modelOverride && shouldRetryGeminiWithFallback(detail)) {
          ...
          if (fallbackModel) {
            logger.warn(`Gemini model ${model} failed, retrying with fallback model ${fallbackModel}: ${detail}`);
      ...
      if (!modelOverride && shouldRetryGeminiWithFallback(detail)) {
        const fallbackModel = findGeminiFallbackModel(account, model);
        if (fallbackModel) {
          logger.warn(`Gemini request for model ${model} threw ${detail}, retrying with fallback model ${fallbackModel}`);
          return this.sendGemini(account, history, fallbackModel);
        }
      }
      throw error;
  ...
  private async fetchOpenAiModels(account: AiAccount) {
  ...
  private async fetchGeminiModels(account: AiAccount) {
    const auth = await this.getGeminiAuth(account);
    const endpoint = account.auth_mode === 'google_oauth'
    ...
    const models = Array.isArray(json?.models)
      ? json.models.map((item: any) => item?.name || item?.displayName).filter(Boolean)
      : [];
  ...
  private async fetchAnthropicModels(account: AiAccount) {
    ...
    if (!response.ok && response.status === 401 && /未提供令牌|missing token/i.test(raw)) {
      ...
      logger.warn(`Anthropic model listing likely lost auth through proxy for ${endpoint}, retrying direct PowerShell request`);
    ...
    const models = Array.isArray(json?.data)
      ? json.data.map((item: any) => item?.id || item?.name || item?.display_name).filter(Boolean)
      : [];
  ...
  private async request(method: 'GET' | 'POST', url: string, headers: Record<string, string>, body?: string) {
    ...
    try {
      ...
      if (proxyTransport.type && isProxyTransportError(error)) {
        logger.warn(`AI fetch failed through proxy for ${method} ${url}, retrying direct: ${detail}`);
        ...
        try {
          ...
          logger.warn(`AI direct retry failed for ${method} ${url}, trying PowerShell fallback: ${formatError(directError)}`);
        }
      } else {
        logger.warn(`AI fetch failed for ${method} ${url}, trying PowerShell fallback: ${detail}`);
      }
  ...
  private async performNodeRequest(
    method: 'GET' | 'POST',
    url: string,
    headers: Record<string, string>,
    body?: string,
    transport?: { agent?: any; dispatcher?: any; type?: string }
  ) {
  ...
  private async requestJson(url: string, headers: Record<string, string>, body: string) {
...
`;
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'muse-ai-'));
    const bodyFile = path.join(tmpDir, 'body.json');
    const headersFile = path.join(tmpDir, 'headers.json');
    fs.writeFileSync(bodyFile, body || '', 'utf8');
    fs.writeFileSync(headersFile, JSON.stringify(headers), 'utf8');
