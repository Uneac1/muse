...
import type {
  ...
  NewspaperReply,
  NewspaperSection,
  NewspaperSectionAiInsight,
  NewspaperSourceStatus,
  NewspaperTopic,
} from '../types';
import { AiAccountModel } from '../models/AiChat';
import { AiChatService } from './AiChatService';
import { NewspaperArticleExtractor } from './newspaper/NewspaperArticleExtractor';
...
interface ParsedSourceResult {
  ...
  status: NewspaperSourceStatus['status'];
  error?: string;
...
interface HackerNewsItemPayload {
  id?: number;
  author?: string;
  created_at?: string;
  points?: number;
  ...
  title?: string;
  url?: string;
  children?: HackerNewsItemPayload[];
}

interface ArticlePrimaryContext {
  kind: 'html' | 'linuxdo';
  html?: string;
  linuxDoPayload?: LinuxDoTopicPayload;
}

interface ArticlePrimaryLoadResult {
  detail: NewspaperArticleDetail;
  retryable: boolean;
  context?: ArticlePrimaryContext;
}
...
export class NewspaperService {
  private aiAccountModel = new AiAccountModel();
  private aiChatService: AiChatService | null = null;
  private articleExtractor = new NewspaperArticleExtractor();
  ...
  private detailCache = new Map<string, TimedCacheEntry<NewspaperArticleDetail>>();
  private articleInsightCache = new Map<string, TimedCacheEntry<NewspaperAiInsight>>();
  private briefingInsightCache = new Map<string, TimedCacheEntry<NewspaperBriefingAiInsight>>();
  private articleCardAiCache = new Map<string, { value: { titleZh: string; summaryZh: string; status: NewspaperArticle['aiCardStatus'] }; expiresAt: number }>();
  private articleCardBatchInflight = new Map<string, Promise<void>>();
  private sourceRetryInflight = new Map<string, Promise<void>>();
  private detailInflight = new Map<string, Promise<NewspaperArticleDetail>>();
  private detailEnhancementInflight = new Map<string, Promise<void>>();
  private articleInsightInflight = new Map<string, Promise<NewspaperAiInsight>>();
  private briefingInsightInflight = new Map<string, Promise<NewspaperBriefingAiInsight>>();
  private detailRetrySchedule = new Map<string, number>();
  private detailWarmScheduled = new Set<string>();
  private autoWarmStarted = false;

  private getFreshCacheValue<T>(cache: Map<string, TimedCacheEntry<T>>, key: string): T | null {
  ...
  private setTimedCacheValue<T>(cache: Map<string, TimedCacheEntry<T>>, key: string, value: T, ttlMs: number): T {
    cache.set(key, { value, expiresAt: Date.now() + ttlMs });
    return value;
  }

  private withInflight<T>(store: Map<string, Promise<T>>, key: string, loader: () => Promise<T>): Promise<T> {
  ...
  private withTimeout<T>(loader: () => Promise<T>, timeoutMs: number, message: string): Promise<T> {
  ...
  private async sendAiPromptWithTimeout(account: AiAccount, prompt: string, timeoutMs: number): Promise<string> {
      ...
      this.getAiChatService().sendMessage(account, [{
        ...
        .then((result) => {
          ...
          const raw = typeof result === 'string'
            ...
            ? result.content
            : String((result as any)?.content?.content || '');
          resolve(raw);
        })
        .catch((error) => {
          clearTimeout(timer);
  ...
  startAutoWarm() {
    if (this.autoWarmStarted) return;
  ...
  getCachedBriefing(limit = DEFAULT_LIMIT): NewspaperBriefing | null {
  ...
  private scheduleVisibleCardWarm(briefing: NewspaperBriefing) {
  ...
  private scheduleVisibleArticleWarm(briefing: NewspaperBriefing) {
    const items = briefing.sections.flatMap((section) => section.items);
    items.forEach((item, index) => {
      const key = item.url.trim();
      if (!key || this.detailWarmScheduled.has(key)) return;

      const cached = this.getFreshCacheValue(this.detailCache, key);
      if (cached?.translationStatus === 'ready' && cached.fulltextStatus === 'ready') return;
      if (cached?.fulltextStatus === 'ready' && cached.translationStatus === 'failed') return;
  ...
  private articleToSeed(item: NewspaperArticle): ArticleSeed {
    return {
      url: item.url,
      source: item.source,
  ...
  private async warmArticleDetailInBackground(seed: ArticleSeed): Promise<NewspaperArticleDetail | null> {
  ...
  private async reloadArticleDetail(seed: ArticleSeed, key: string, fallback?: NewspaperArticleDetail): Promise<NewspaperArticleDetail> {
    return this.withInflight(this.detailInflight, key, async () => {
      try {
        const primary = await this.loadPrimaryArticleDetail(seed);
        this.detailRetrySchedule.delete(key);
  ...
  async *streamArticleRead(seed: ArticleSeed): AsyncGenerator<NewspaperArticleStreamEvent> {
    const key = seed.url.trim();
    if (!key) {
    ...
    let detail: NewspaperArticleDetail;
    try {
      const primary = await this.loadPrimaryArticleDetail(seed);
      detail = {
        ...primary.detail,
  ...
  private async buildFallbackArticleDetail(seed: ArticleSeed): Promise<NewspaperArticleDetail> {
    ...
    const [titleTranslation, summaryTranslation] = await Promise.all([
      ...
      summary ? this.translateTextDetailedOptional(summary, [], 'summary') : Promise.resolve({ text: '', mode: 'fallback' as const }),
    ]);
  ...
  private async loadPrimaryArticleDetail(seed: ArticleSeed): Promise<ArticlePrimaryLoadResult> {
    const linuxDoPrimary = await this.loadLinuxDoPrimaryArticleDetail(seed);
    if (linuxDoPrimary) return linuxDoPrimary;
    ...
    return this.withTimeout(async () => {
      ...
      const [titleTranslation, summaryTranslation] = hasFulltext
        ? await Promise.all([
          this.translateTextDetailedOptional(title, [], 'title'),
          summary ? this.translateTextDetailedOptional(summary, [], 'summary') : Promise.resolve<TranslationResult>({ text: '', mode: 'fallback' }),
        ])
        : [
          { text: seed.titleZh || '', mode: 'fallback' as const },
          { text: '', mode: 'fallback' as const },
        ];
  ...
  private async loadLinuxDoPrimaryArticleDetail(seed: ArticleSeed): Promise<ArticlePrimaryLoadResult | null> {
    const topicPayload = await this.fetchLinuxDoTopicPayload(seed.url);
    if (!topicPayload) return null;
    ...
    const [titleTranslation, summaryTranslation] = hasFulltext
      ? await Promise.all([
        ...
        summaryCandidate ? this.translateTextDetailedOptional(summaryCandidate, ['community', 'discussion'], 'summary') : Promise.resolve<TranslationResult>({ text: '', mode: 'fallback' }),
      ])
      : [
        { text: seed.titleZh || '', mode: 'fallback' as const },
        { text: '', mode: 'fallback' as const },
  ...
  private enhanceArticleDetailInBackground(seed: ArticleSeed, current: NewspaperArticleDetail, context: ArticlePrimaryContext) {
    const key = seed.url.trim();
    if (!key) return;
  ...
  private async loadEnhancedArticleDetail(
    seed: ArticleSeed,
    current: NewspaperArticleDetail,
    context: ArticlePrimaryContext,
  ...
  private buildEnhancedFailureDetail(current: NewspaperArticleDetail): NewspaperArticleDetail {
  ...
  private buildPreviewOriginalContent(title: string, summary: string): string {
    return [title.trim(), summary.trim()].filter(Boolean).join('\n\n');
  }

  private buildPreviewTranslatedContent(titleZh: string, summaryZh: string, fallbackTitle: string, fallbackSummary: string): string {
    return [titleZh.trim(), summaryZh.trim()].filter(Boolean).join('\n\n');
  }

  private splitReadableParagraphs(text: string): string[] {
  ...
  private normalizePreviewOnlyDetail(detail: NewspaperArticleDetail): NewspaperArticleDetail {
    if (detail.fulltextStatus === 'ready') return detail;
    ...
    if (!isPreviewOnly && detail.originalContent.trim()) return detail;
  ...
  private hasReadableFulltext(text: string): boolean {
    return this.articleExtractor.hasReadableFulltext(text);
  }

  private shouldTranslateBody(text: string): boolean {
    const cleaned = this.articleExtractor.cleanArticleText(text || '');
    return Boolean(cleaned) && !this.looksChinese(cleaned);
  }

  private async fetchLinuxDoTopicPayload(articleUrl: string): Promise<LinuxDoTopicPayload | null> {
    try {
      const topicUrl = this.normalizeLinuxDoTopicUrl(articleUrl);
      if (!topicUrl) return null;
      return await this.fetchJsonWithFallback<LinuxDoTopicPayload>(`${topicUrl}.json`, 12000);
    } catch {
      return null;
    }
  }

  private async extractLinuxDoRepliesFromPayload(
    payload: LinuxDoTopicPayload | undefined,
    articleUrl: string,
  ): Promise<NewspaperReply[]> {
    if (!payload) return [];
    ...
    return Promise.all(replies.map(async (post: any) => {
      const content = this.articleExtractor.cleanArticleText(this.articleExtractor.htmlToText(post?.cooked || '')).slice(0, 1200);
      const translated = content ? await this.translateTextDetailed(content, ['community', 'discussion'], 'body') : { text: '', mode: 'fallback' as const };
      ...
      return {
        ...
        url: postUrl,
      } satisfies NewspaperReply;
    }));
  }

  private scheduleDetailRetry(seed: ArticleSeed) {
    const key = seed.url.trim();
    if (!key || this.detailInflight.has(key)) return;
    const nextRetryAt = Date.now() + DETAIL_RETRY_BACKOFF_MS;
    this.detailRetrySchedule.set(key, nextRetryAt);
    setTimeout(() => {
      if ((this.detailRetrySchedule.get(key) || 0) > Date.now()) return;
  ...
  async generateArticleInsight(seed: ArticleSeed & { accountId?: number | null }): Promise<NewspaperAiInsight> {
    ...
    return this.withInflight(this.articleInsightInflight, cacheKey, async () => {
      ...
      let accountPool: AiAccount[] = [];
      let account: AiAccount | null = null;
      try {
      ...
      let parsed: Omit<NewspaperAiInsight, 'accountId' | 'accountName' | 'model' | 'generatedAt' | 'status' | 'degradedReason'> | null = null;
      let cacheTtl = AI_INSIGHT_TTL_MS;
      const attemptedAccounts: string[] = [];
      let degradedReason: string | null = null;

      let lastError: any = accountPool.length === 0 ? new Error('没有可用的 AI 账号。先去 AI 对话页面启用一个账号。') : null;
      ...
      const result = {
        ...
        generatedAt: new Date().toISOString(),
      } satisfies NewspaperAiInsight;
      return this.setTimedCacheValue(this.articleInsightCache, cacheKey, result, cacheTtl);
    });
  }

  private buildFallbackArticleInsightSummary(detail: NewspaperArticleDetail): string {
  ...
  async generateBriefingInsight(options?: { accountId?: number | null; limit?: number; refresh?: boolean; query?: string }): Promise<NewspaperBriefingAiInsight> {
    ...
    return this.withInflight(this.briefingInsightInflight, cacheKey, async () => {
      ...
      let parsed: Omit<NewspaperBriefingAiInsight, 'accountId' | 'accountName' | 'model' | 'generatedAt'> | null = null;
      let cacheTtl = AI_INSIGHT_TTL_MS;
      const attemptedAccounts: string[] = [];
      ...
      const result: NewspaperBriefingAiInsight = {
        accountId: account?.id || 0,
        accountName: account ? (account.name || account.provider) : 'Fallback',
        model: account?.model || 'fallback',
  ...
  private async buildSection(config: SectionConfig): Promise<NewspaperSection> {
    ...
    const status: NewspaperSection['status'] = items.length === 0
      ? 'empty'
  ...
  private async fetchSourceItems(source: FeedSource, topics: NewspaperTopic[]): Promise<ParsedSourceResult> {
    ...
    try {
      ...
      const result = {
        ...
        status: cached?.value.items?.length ? 'stale' as const : 'failed' as const,
  ...
  private scheduleSourceRetry(source: FeedSource, topics: NewspaperTopic[]) {
    if (this.sourceRetryInflight.has(source.url)) return;
  ...
  private async refreshSourceItems(source: FeedSource, topics: NewspaperTopic[]): Promise<ParsedSourceResult> {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);
    try {
      const xml = await this.fetchSourceXml(source, controller.signal);
      ...
      const result = {
        ...
        status: previous?.items?.length ? 'stale' as const : 'failed' as const,
        error: error?.message || 'Feed fetch failed',
...
`.trim();
  ...
  private async enrichArticlesWithAiCards(config: SectionConfig, items: NewspaperArticle[]): Promise<NewspaperArticle[]> {
    ...
    const output = items.map((item, index) => {
      ...
      return {
        ...
        titleZh: '',
        summaryZh: '',
        aiCardStatus: 'pending' as NewspaperArticle['aiCardStatus'],
      };
    });
    ...
    const rewrites = await this.withTimeout(
      () => this.generateAiArticleCardBatch(accountPool, config, targets),
      AI_CARD_TIMEOUT_MS,
      `Article card AI timed out after ${AI_CARD_TIMEOUT_MS}ms`,
    ).catch(() => []);
    if (rewrites.length === 0) {
      const failedStatus = (accountPool.length > 0 ? 'failed' : 'pending') as NewspaperArticle['aiCardStatus'];
    ...
    return output.map((item) => {
      ...
      if (!matched) {
        return {
          ...
          titleZh: '',
          summaryZh: '',
          aiCardStatus: 'failed' as NewspaperArticle['aiCardStatus'],
        };
      }
      const titleZh = this.finalizeCardHeadline(matched.titleZh);
      const summaryZh = this.finalizeCardSummary(matched.summaryZh);
      const status = (titleZh || summaryZh ? 'ready' : 'failed') as NewspaperArticle['aiCardStatus'];
      this.articleCardAiCache.set(this.getArticleCardCacheKey(item), {
  ...
  private attachAiCardsFromCache(items: NewspaperArticle[]): NewspaperArticle[] {
    return items.map((item) => {
      const cached = this.articleCardAiCache.get(this.getArticleCardCacheKey(item));
      if (cached && cached.expiresAt > Date.now()) {
        return {
  ...
  private warmArticleCardsInBackground(config: SectionConfig, items: NewspaperArticle[]) {
    ...
    if (this.articleCardBatchInflight.has(batchKey)) return;
    setTimeout(() => {
  ...
  private getArticleCardCacheKey(item: Pick<NewspaperArticle, 'url' | 'title' | 'summary'>) {
    return `${item.url}::${item.title}::${item.summary}`.toLowerCase();
  }

  private async generateAiArticleCardBatch(
    accountPool: AiAccount[],
    config: SectionConfig,
    items: NewspaperArticle[],
  ): Promise<Array<{ url: string; titleZh: string; summaryZh: string }>> {
  ...
  private parseArticleCardBatch(raw: string, fallbackItems: NewspaperArticle[]): Array<{ url: string; titleZh: string; summaryZh: string }> {
    try {
      const jsonText = raw.match(/\{[\s\S]*\}/)?.[0];
      if (!jsonText) return this.parseLooseArticleCardBatch(raw, fallbackItems);
      const parsed = JSON.parse(jsonText) as { items?: Array<{ url?: string; titleZh?: string; summaryZh?: string }> };
      ...
      return parsed.items
        ...
        .filter((item): item is { url: string; titleZh: string; summaryZh: string } => !!item);
    } catch {
      return this.parseLooseArticleCardBatch(raw, fallbackItems);
    }
  }

  private parseLooseArticleCardBatch(raw: string, fallbackItems: NewspaperArticle[]): Array<{ url: string; titleZh: string; summaryZh: string }> {
  ...
  private finalizeCardHeadline(candidate: string) {
    ...
    if (!cleaned || isBadMixedHeadline || !this.looksChinese(cleaned)) return '';
    return this.polishChineseLine(cleaned, 44);
  }

  private finalizeCardSummary(candidate: string) {
    const cleaned = this.cleanFeedSummary(candidate).trim();
    if (!cleaned || /^(Article URL|Comments URL|Listen now|Read more|立即观看)/i.test(cleaned)) return '';
    const chinese = this.toChineseBrief(cleaned, 'summary');
    if (!this.looksChinese(chinese)) return '';
    return chinese;
  }
  ...
  private async translateTextDetailedOptional(text: string, matchedKeywords: string[] = [], mode: 'title' | 'summary' | 'body' = 'summary'): Promise<TranslationResult> {
    try {
  ...
  private async translateViaAi(text: string, mode: 'title' | 'summary' | 'body'): Promise<string> {
...
`.trim();
  ...
  private async extractReplies(url: string, html: string): Promise<NewspaperReply[]> {
    if (/linux\.do/i.test(url)) {
      const linuxDoReplies = await this.fetchLinuxDoReplies(url);
      if (linuxDoReplies.length > 0) return linuxDoReplies;
    }
  ...
  private async fetchLinuxDoReplies(articleUrl: string): Promise<NewspaperReply[]> {
  ...
  private normalizeLinuxDoTopicUrl(url: string): string | null {
    try {
      ...
      if (!topicPath.startsWith('/t/')) return null;
  ...
  private parseAiInsight(raw: string): Omit<NewspaperAiInsight, 'accountId' | 'accountName' | 'model' | 'generatedAt' | 'status' | 'degradedReason'> {
    const fallback = {
    ...
    try {
      ...
      const parsed = JSON.parse(jsonText) as Partial<Record<'summary' | 'takeaways' | 'risks' | 'questions' | 'actions', unknown>>;
      return {
  ...
  private normalizeInsightList(value: unknown, fallback: string[], limit = 4): string[] {
  ...
  private buildBriefingCoverage(sections: NewspaperSection[], highlights: string[]) {
    return {
  ...
  private buildGlobalHighlights(sectionInsights: NewspaperSectionAiInsight[], sections: NewspaperSection[], limit = 10) {
    const output: string[] = [];
    ...
    for (const section of sectionInsights) {
      ...
      if (candidate) push(`${section.title}：${candidate}`);
    }
  ...
  private parseBriefingInsight(raw: string, sections: NewspaperSection[]): Omit<NewspaperBriefingAiInsight, 'accountId' | 'accountName' | 'model' | 'generatedAt'> {
    const fallbackSections: NewspaperSectionAiInsight[] = sections.map((section) => this.buildSectionInsightFallback(section));
    ...
    try {
      const jsonText = raw.match(/\{[\s\S]*\}/)?.[0];
      if (!jsonText) return this.parseLooseBriefingInsight(raw, sections, fallback);
      const parsed = JSON.parse(jsonText) as Partial<Record<'headline' | 'summary' | 'highlights' | 'watchlist' | 'opportunities' | 'sections', unknown>>;
      const aiSections = Array.isArray(parsed.sections)
        ? parsed.sections
            .map((item: any) => {
              ...
              } satisfies NewspaperSectionAiInsight;
            })
            .filter((item): item is NewspaperSectionAiInsight => !!item)
        : [];

      const resolvedSections = aiSections.length > 0 ? aiSections : fallback.sections;
      const resolvedHighlights = this.normalizeInsightList(
        parsed.highlights,
        this.buildGlobalHighlights(resolvedSections, sections, 10),
        10,
      ).map((item) => this.toChineseBrief(item, 'sentence'));
      return {
  ...
  private parseLooseBriefingInsight(
    ...
    fallback: Omit<NewspaperBriefingAiInsight, 'accountId' | 'accountName' | 'model' | 'generatedAt'>,
  ): Omit<NewspaperBriefingAiInsight, 'accountId' | 'accountName' | 'model' | 'generatedAt'> {
  ...
  private parseBriefingSectionInsight(raw: string, section: NewspaperSection): NewspaperSectionAiInsight {
    const fallback = this.buildSectionInsightFallback(section);

    try {
      const jsonText = raw.match(/\{[\s\S]*\}/)?.[0];
      if (!jsonText) return this.parseLooseSectionInsight(raw, section, fallback);
      const parsed = JSON.parse(jsonText) as Partial<Record<'summary' | 'topSignals', unknown>>;
      const insight = {
  ...
  private parseLooseSectionInsight(raw: string, section: NewspaperSection, fallback: NewspaperSectionAiInsight): NewspaperSectionAiInsight {
    const lines = String(raw || '')
      .replace(/```[a-z]*|```/gi, ' ')
      .replace(/\r/g, '')
      .split('\n')
  ...
  private async generateBriefingInsightBySections(
    account: AiAccount,
    sections: NewspaperSection[],
    generatedAt: string,
  ): Promise<Omit<NewspaperBriefingAiInsight, 'accountId' | 'accountName' | 'model' | 'generatedAt'>> {
  ...
  private resolveActiveAiAccount(accountId: number | null) {
  ...
  private resolveAiAccountPool(accountId: number | null): AiAccount[] {
    const activeAccounts = this.aiAccountModel
      .list()
      .filter((account) => account.status === 'active')
      .sort((a, b) => {
  ...
  private scoreNewspaperAiAccount(account: AiAccount): number {
    const provider = String(account.provider || '').toLowerCase();
    const model = String(account.model || '').toLowerCase();
    const name = String(account.name || '').toLowerCase();
    const text = `${provider} ${model} ${name}`;

    if (/gpt-5(\.|\b)|codex|claude-opus|claude-sonnet|gpt-4\.1|o3|o4/.test(text)) return 100;
    if (/gemini-2\.5-pro|gemini-pro/.test(text)) return 85;
    if (/gemini-2\.5-flash-lite|flash-lite/.test(text)) return 40;
    if (/gemini-2\.5-flash|gemini-2\.0-flash|flash/.test(text)) return 55;
    return 70;
  }
  ...
  private cleanFeedSummary(text: string): string {
  ...
  private polishChineseLine(text: string, maxLength: number): string {
    return text
      .replace(/\s+/g, ' ')
      .replace(/[：:]\s*$/, '')
      .slice(0, maxLength)
  ...
  private buildFallbackChineseTitle(text: string, matchedKeywords: string[]): string {
    ...
    if (this.looksChinese(normalized)) return normalized;
    if (normalized) {
      return normalized;
    }
  ...
  private buildFallbackChineseSummary(text: string, matchedKeywords: string[]): string {
    ...
    if (this.looksChinese(cleaned)) return cleaned;
    const subject = this.polishChineseLine(this.cleanFeedSummary(text), 40);
    if (localizedKeywords.length > 0) {
  ...
  private buildDisplayHeadline(title: string, summary: string, matchedKeywords: string[], localizedTitle?: string): string {
  ...
  private buildDisplaySummary(summary: string, title: string, matchedKeywords: string[], localizedSummary?: string): string {
  ...
  private toChineseBrief(text: string, mode: 'summary' | 'sentence'): string {
    const cleaned = this.cleanFeedSummary(text);
  ...
  private buildSectionInsightFallback(section: NewspaperSection): NewspaperSectionAiInsight {
    ...
    return {
      sectionId: section.id,
      title: section.title,
      summary: this.polishChineseLine(
        summaryParts.length > 0
          ? `${section.title}今天最值得先看的是：${summaryParts.join('；')}`
          : `${section.title}暂时还没有足够信号可提炼。`,
        160,
      ),
  ...
  private buildArticleSignal(item: NewspaperArticle): string {
    ...
    const localizedPreferred = preferred && !this.looksChinese(preferred)
      ...
      : preferred;
    if (localizedPreferred && !this.isWeakSnippet(localizedPreferred)) return this.polishChineseLine(localizedPreferred, 56);
  ...
  private buildArticleEssence(item: NewspaperArticle): string {
    const summary = this.cleanFeedSummary(item.summaryZh || item.summary || '');
    const title = this.cleanFeedSummary(item.titleZh || item.title || '');
    const preferred = this.extractReadableSnippet(summary) || this.extractReadableSnippet(title);
    const localizedPreferred = preferred && !this.looksChinese(preferred)
      ? this.localizeText(preferred, item.matchedKeywords || [], 'summary')
      : preferred;
    if (localizedPreferred && !this.isWeakSnippet(localizedPreferred)) return this.polishChineseLine(localizedPreferred, 88);
    if (title) {
  ...
  private extractReadableSnippet(text: string): string {
    const cleaned = this.cleanFeedSummary(text)
  ...
  private isWeakSnippet(text: string): boolean {
    const cleaned = this.cleanFeedSummary(text);
    return !cleaned
  ...
  private isLowSignalArticle(item: Pick<NewspaperArticle, 'title' | 'titleZh' | 'summary' | 'summaryZh'>): boolean {
    const title = this.cleanFeedSummary(item.titleZh || item.title || '');
    const summary = this.cleanFeedSummary(item.summaryZh || item.summary || '');
    return this.isPromotionalOrSensitiveNoise(title) || this.isPromotionalOrSensitiveNoise(summary);
  }

  private isPromotionalOrSensitiveNoise(text: string): boolean {
    const cleaned = this.cleanFeedSummary(text);
    return /本帖使用社区开源推广|符合推广要求|返一下|邀请码|优惠码|抽奖|额度的api密钥|分享一个\d+刀的额度|已脱敏密钥|已脱敏令牌|已脱敏串|接码|自动化古法注册|代充|成品号|羊毛/i.test(cleaned);
  }

  private adjustDisplayScore(item: NewspaperArticle): number {
  ...
  private isLowSignalSectionInsight(section: NewspaperSectionAiInsight): boolean {
  ...
  private isLowSignalBriefingInsight(
    insight: Omit<NewspaperBriefingAiInsight, 'accountId' | 'accountName' | 'model' | 'generatedAt'> | null,
  ): boolean {
  ...
  private async fetchJsonWithFallback<T>(url: string, timeoutMs = ARTICLE_TIMEOUT_MS): Promise<T> {
    const controller = new AbortController();
    ...
    try {
      ...
      if (!res.ok) throw new Error(`JSON request failed: ${res.status}`);
