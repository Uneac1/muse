...
exports.AiMessageModel = exports.AiThreadModel = exports.AiAccountModel = void 0;
...
class AiAccountModel {
    list() {
        return database_1.default.prepare('SELECT * FROM ai_accounts ORDER BY priority_rank ASC, updated_at DESC, id DESC').all();
    }
    getById(id) {
        return database_1.default.prepare('SELECT * FROM ai_accounts WHERE id = ?').get(id);
    }
    create(data) {
    ...
    `).run(provider, data.auth_mode || 'api_key', data.name || '', data.api_key || '', data.base_url || defaults.baseUrl, data.model || defaults.model, data.priority_rank || 1, data.oauth_client_id || '', data.oauth_client_secret || '', data.oauth_refresh_token || '', data.oauth_email || '', data.oauth_project_id || '', data.system_prompt || '', data.remark || '', data.status || 'active', 'never', '[]', 'unknown');
    ...
    update(id, data) {
      ...
      WHERE id = ?
    `).run(payload.provider, payload.auth_mode, payload.name, payload.api_key, payload.base_url, payload.model, payload.priority_rank, payload.oauth_client_id, payload.oauth_client_secret, payload.oauth_refresh_token, payload.oauth_email, payload.oauth_project_id, payload.system_prompt, payload.remark, payload.status, id);
        return this.getById(id);
    }
    delete(id) {
        const result = database_1.default.prepare('DELETE FROM ai_accounts WHERE id = ?').run(id);
...
exports.AiAccountModel = AiAccountModel;
class AiThreadModel {
...
class AiMessageModel {
    ...
    create(threadId, role, content) {
        const result = database_1.default.prepare(`
      INSERT INTO ai_messages (thread_id, role, content)
      VALUES (?, ?, ?)
    `).run(threadId, role, content);
        return database_1.default.prepare('SELECT * FROM ai_messages WHERE id = ?').get(result.lastInsertRowid);
    }
}
