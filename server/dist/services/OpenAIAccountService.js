...
function buildResetLabelFromSeconds(seconds) {
...
function buildWindowCard(key, label, usedPercent, resetAfterSeconds, tone = 'slate') {
...
class OpenAIAccountService {
    createSession(redirectUri) {
        const state = crypto_1.default.randomBytes(32).toString('hex');
        const codeVerifier = crypto_1.default.randomBytes(64).toString('hex');
        const codeChallenge = encodeBase64Url(crypto_1.default.createHash('sha256').update(codeVerifier).digest());
    ...
    parseTokenInfo(accessToken, refreshToken, idToken) {
        ...
        return {
            ...
            chatgptAccountId: String(idClaims?.['https://api.openai.com/auth']?.chatgpt_account_id ||
    ...
    async fetchWhamSnapshot(sourceUrl, accessToken, explicitAccountId) {
        ...
        const usageSections = cards.length
            ? [{
                    title: '官方额度详情',
    ...
    async requestToken(payload, fallbackRefreshToken = '') {
...
//# sourceMappingURL=OpenAIAccountService.js.map