...
class NewspaperService {
    ...
    async getArticleDetail(seed) {
        ...
        try {
            ...
            const title = this.decodeEntities(this.extractMetaContent(html, 'property', 'og:title')) ||
    ...
    async buildFallbackArticleDetail(seed) {
        ...
        const [titleTranslation, summaryTranslation] = await Promise.all([
            ...
            summary ? this.translateTextDetailed(summary, [], 'summary') : Promise.resolve({ text: '', mode: 'fallback' }),
        ]);
    ...
    async buildSection(config) {
        ...
        const status = items.length === 0 ? 'empty' : successCount === config.sources.length ? 'ok' : 'partial';
        return {
            id: config.id,
            emoji: config.emoji,
            title: config.title,
...
`.trim();
    ...
    localizeText(text, matchedKeywords, mode) {
        ...
        if (mode === 'body')
            return localized;
        if (this.looksChinese(localized))
            return localized;
    ...
    extractMetaContent(html, attr, key) {
    ...
    extractReadableContent(html, fallback) {
    ...
    extractElementsByTag(html, tag) {
        return html.match(new RegExp(`<${tag}\\b[^>]*>[\\s\\S]*?<\\/${tag}>`, 'gi')) || [];
    }
    extractElementsByClassHint(html, hints) {
        const matches = [];
    ...
    cleanArticleText(text) {
        ...
        return text
            ...
            if (!line || /^share$/i.test(line) || /^comments?$/i.test(line))
                return false;
            if (noisePatterns.some((pattern) => pattern.test(normalizedLine)))
                return false;
            if (/^•\s*$/.test(line))
                return false;
            if (/^•\s+/.test(line) && normalizedLine.length <= 40)
                return false;
            return true;
        })
            .join('\n')
