...
const LinuxDoConnectService_1 = require("./LinuxDoConnectService");
class IntegrationService {
    linuxDoConnectService = new LinuxDoConnectService_1.LinuxDoConnectService();
    cache = new Map();
    inflight = new Map();
    notionInsightsSampleLimit = 48;
    externalRequestConcurrency = 4;
    notionSearchPageLimit = 10;
    notionContentPageLimit = 20;
    maskToken(token) {
        if (!token)
            return '';
        if (token.length <= 8)
    ...
    async fetchGitHubRepoExtras(token, repos) {
        ...
        const loadForTargets = (loader) => this.mapWithConcurrency(targets, this.externalRequestConcurrency, (repo) => this.safeRequest(() => loader(repo), []));
    ...
    async fetchReadableNotionBlocks(token, blockId, depth = 0) {
        ...
        const blocks = await this.mapWithConcurrency(rawBlocks, this.externalRequestConcurrency, async (item) => {
    ...
    async fetchCloudflareZoneDetails(token, zones) {
        ...
        const results = await this.mapWithConcurrency(targets, this.externalRequestConcurrency, async (zone) => {
    ...
    async fetchCloudflarePagesProjects(token, accounts) {
        const results = await this.mapWithConcurrency(accounts.slice(0, 20), this.externalRequestConcurrency, (account) => this.safeRequest(() => this.fetchCloudflarePaged(`/accounts/${account.id}/pages/projects`, token), []));
        return results.flat();
    }
    async fetchCloudflareWorkerScripts(token, accounts) {
        const results = await this.mapWithConcurrency(accounts.slice(0, 20), this.externalRequestConcurrency, (account) => this.safeRequest(() => this.fetchCloudflarePaged(`/accounts/${account.id}/workers/scripts`, token), []));
        return results.flat();
    }
    async fetchCloudflareRulesets(token, accounts, zones) {
        const accountRulesets = await this.mapWithConcurrency(accounts.slice(0, 20), this.externalRequestConcurrency, (account) => this.safeRequest(() => this.fetchCloudflarePaged(`/accounts/${account.id}/rulesets`, token), []));
        const zoneRulesets = await this.mapWithConcurrency(zones.slice(0, 50), this.externalRequestConcurrency, (zone) => this.safeRequest(() => this.fetchCloudflarePaged(`/zones/${zone.id}/rulesets`, token), []));
    ...
    async fetchLinuxDoData(record, options) {
    ...
    async fetchNotionInsights(record, options) {
        const token = record.token;
        return this.withCache(this.cacheKey('notion-insights', token, `sample-${this.notionInsightsSampleLimit}`), 3 * 60 * 1000, !!options?.force, async () => {
            ...
            logger_1.default.info(`Notion insights sampled ${sampledPages.length}/${summary.pages.length} pages and ${blocks.length} blocks`);
            return result;
        });
    }
