...
exports.PersonalOSService = void 0;
...
const PersonalOS_1 = require("../models/PersonalOS");
...
const YmailService_1 = require("./YmailService");
const accountModel = new Account_1.AccountModel();
const aiAccountModel = new AiChat_1.AiAccountModel();
const mailModel = new MailCache_1.MailCacheModel();
const proxyModel = new Proxy_1.ProxyModel();
const tokenModel = new TokenAccount_1.TokenAccountModel();
const integrationTokenModel = new IntegrationToken_1.IntegrationTokenModel();
const ruleModel = new PersonalOS_1.PersonalOsRuleModel();
const memoryModel = new PersonalOS_1.PersonalMemoryModel();
const actionStateModel = new PersonalOS_1.PersonalActionStateModel();
...
let newspaperServiceSingleton = null;
function getNewspaperService() {
    if (!newspaperServiceSingleton) {
        newspaperServiceSingleton = require('./NewspaperService').newspaperService;
    }
    return newspaperServiceSingleton;
}
const VALID_RULE_SCOPES = new Set(['today', 'inbox', 'alert']);
const VALID_RULE_TRIGGER_TYPES = new Set([
...
const VALID_MEMORY_KINDS = new Set(['note', 'project', 'risk', 'preference']);
...
class PersonalOSService {
    ...
    async buildWorkspace() {
        ...
        const tokenAccounts = tokenModel.list().map((item) => ({
            ...item,
    ...
    setActionState(actionId, status, note = '') {
        if (status === 'active') {
    ...
    async planMemoryWithAi(input) {
        ...
        const suggestions = Array.isArray(parsed?.suggestions)
            ? parsed.suggestions.map((item) => this.normalizeAiSuggestion(item)).filter(Boolean)
            : [];
        return {
    ...
    async planAccountsWithAi(input) {
        const accounts = accountModel.getAll();
        const history = [
            {
                id: 0,
        ...
        const parsed = parseJsonLoose(raw);
        const suggestions = Array.isArray(parsed?.suggestions)
            ? parsed.suggestions.map((item) => this.normalizeAiAccountSuggestion(item)).filter(Boolean)
            : [];
        return {
            requestedAccountId: requestedAccount?.id ?? input?.accountId ?? null,
            requestedAccountName: requestedAccount ? (requestedAccount.name || requestedAccount.provider) : undefined,
    ...
    applyAccountAiPlan(input) {
        ...
        for (const suggestion of suggestions.slice(0, 20)) {
            ...
            if (!current)
                continue;
            const nextPayload = {};
            if (normalized.mode && normalized.mode !== current.mode)
                nextPayload.mode = normalized.mode;
            if (normalized.status && normalized.status !== current.status)
                nextPayload.status = normalized.status;
            if (normalized.remark !== undefined && normalized.remark !== current.remark)
                nextPayload.remark = normalized.remark;
            ...
            if (Array.isArray(normalized.tag_names)) {
                ...
                const currentTagIds = (current.tags || []).map((tag) => tag.id).sort((a, b) => a - b);
    ...
    async planProxiesWithAi(input) {
        ...
        const parsed = parseJsonLoose(raw);
        const suggestions = Array.isArray(parsed?.suggestions)
            ? parsed.suggestions.map((item) => this.normalizeAiProxySuggestion(item)).filter(Boolean)
            : [];
        return {
            requestedAccountId: requestedAccount?.id ?? input?.accountId ?? null,
            requestedAccountName: requestedAccount ? (requestedAccount.name || requestedAccount.provider) : undefined,
    ...
    async planTokensWithAi(input) {
        ...
        const parsed = parseJsonLoose(raw);
        const suggestions = Array.isArray(parsed?.suggestions)
            ? parsed.suggestions.map((item) => this.normalizeAiTokenSuggestion(item)).filter(Boolean)
            : [];
        return {
            requestedAccountId: requestedAccount?.id ?? input?.accountId ?? null,
            requestedAccountName: requestedAccount ? (requestedAccount.name || requestedAccount.provider) : undefined,
    ...
    applyTokenAiPlan(input) {
        ...
        for (const suggestion of suggestions.slice(0, 20)) {
            ...
            if (normalized.note !== undefined && normalized.note !== current.note)
    ...
    async loadIntegrations() {
        ...
        const ymailRecord = integrationTokenModel.get('ymail') || (config_1.config.defaultYmailAdminPassword
            ? {
                provider: 'ymail',
    ...
    collectBaseActions(input) {
        ...
        for (const account of input.aiAccounts.filter((item) => item.status === 'error' || (item.status !== 'inactive' && item.last_test_status === 'failed'))) {
        ...
        for (const account of input.aiAccounts.filter((item) => item.status === 'active' && item.last_test_status === 'never').slice(0, 2)) {
            items.push({
        ...
        for (const token of input.tokenAccounts.filter((item) => {
            const card = item.snapshot?.cards?.find((entry) => typeof entry.remainingPct === 'number');
            return typeof card?.remainingPct === 'number' && card.remainingPct <= 20;
        })) {
            const quota = token.snapshot.cards.find((entry) => typeof entry.remainingPct === 'number');
            items.push({
        ...
        for (const rule of rules.filter((item) => item.is_enabled)) {
            if (rule.trigger_type === 'token_low_remaining_pct') {
                ...
                for (const token of context.tokenAccounts) {
                    const card = token.snapshot?.cards?.find((entry) => typeof entry.remainingPct === 'number');
                ...
                if (hit)
                    ruleModel.markTriggered(rule.id);
            }
            ...
            if (rule.trigger_type === 'keyword_in_news' && context.newspaper) {
                ...
                if (!keyword)
                    continue;
                const matches = context.newspaper.sections.flatMap((section) => section.items).filter((item) => [item.title, item.titleZh, item.summary, item.summaryZh].join(' ').toLowerCase().includes(keyword)).slice(0, 4);
    ...
    buildCommandCenter(input) {
        const items = [...this.moduleCommands];
        for (const account of input.accounts.slice(0, 20)) {
    ...
    buildTodaySummary(priorities, alerts, highlights) {
    ...
    buildTodayNextStep(item) {
        if (!item)
            return '/inbox';
        if (item.source === 'mail')
            return '/inbox';
        if (item.source === 'news')
            return item.actionPath || '/newspaper';
        if (item.source === 'notion')
            return '/notion';
        if (item.actionPath === '/tokens')
            return '/inbox';
        return item.actionPath || '/inbox';
    }
    buildNewspaperReaderPath(input) {
        ...
        if (input.publishedAt)
            params.set('publishedAt', input.publishedAt);
        if (input.title)
            params.set('title', input.title);
        if (input.titleZh)
            params.set('titleZh', input.titleZh);
        if (input.summary)
            params.set('summary', input.summary);
        if (input.summaryZh)
            params.set('summaryZh', input.summaryZh);
    ...
    todayWeight(item, preferredSources, preferredPaths) {
        let score = this.weight(item);
        if (preferredSources.has(item.source))
            score += 500;
        if (preferredPaths.has(item.actionPath))
            score += 260;
        if (item.source === 'mail')
            score += 120;
        if (item.actionPath === '/inbox')
            score += 100;
        if (item.source === 'news')
            score += 100;
        if (item.source === 'notion')
            score += 90;
        if (item.source === 'tokens')
            score -= 420;
        if (item.actionPath === '/tokens')
            score -= 160;
        return score;
    }
    todayHighlightWeight(item, preferredSources) {
        let score = this.weight(item);
        if (preferredSources.has(item.source))
            score += 320;
        if (item.source === 'mail')
    ...
    async safe(fn, fallback, timeoutMs = 8000) {
        let timer = null;
        try {
    ...
    applyActionStates(items, states) {
    ...
    listAiAccountCandidates(accountId) {
    ...
    getAiAccountRankScore(account) {
        if (account.last_test_status === 'success')
            return 0;
        if (account.status === 'active')
            return 1;
        return 2;
    }
    isKnownBrokenAiFallbackCandidate(account) {
        if (account.last_test_status === 'success')
            return false;
    ...
    async sendStructuredAiPrompt(accountId, history, prompt) {
                ...
                try {
                    ...
                    const raw = typeof aiResult === 'string'
                        ...
                        : typeof aiResult?.content === 'string'
                            ? aiResult.content
                            : String(aiResult?.content?.content || '');
    ...
    shouldRetrySameAiAccount(error) {
        const message = String(error?.message || error || '').toLowerCase();
        if (!message)
            return false;
        return [
    ...
    describeAiFallbackReason(error) {
        const message = String(error?.message || error || '').trim();
        if (!message)
            return '首选账号请求失败，已切换到后备账号。';
    ...
    normalizeAiPromptError(error, requestedAccount, hasFallbackCandidate = false) {
    ...
    buildMemoryManagerPrompt(memory, inbox, focus) {
    ...
    buildAccountManagerPrompt(accounts, focus) {
    ...
    buildProxyManagerPrompt(proxies, focus) {
    ...
    buildTokenManagerPrompt(accounts, focus) {
    ...
    normalizeAiSuggestion(input) {
        const type = ['create', 'update', 'resolve'].includes(String(input?.type)) ? String(input.type) : null;
        if (!type)
            return null;
        ...
        if (type !== 'create' && !Number.isFinite(targetId))
            return null;
        if (type !== 'resolve' && (!title || !content))
            return null;
        return {
            type,
    ...
    normalizeAiAccountSuggestion(input) {
        if (String(input?.type) !== 'update')
            return null;
        ...
        const tagNames = Array.isArray(input?.tag_names)
            ? [...new Set(input.tag_names.map((item) => String(item || '').trim()).filter(Boolean))].slice(0, 4)
            : undefined;
        if (!mode && !status && remark === undefined && !tagNames)
            return null;
        return {
            type: 'update',
            target_id: targetId,
    ...
    normalizeAiProxySuggestion(input) {
        if (String(input?.type) !== 'update')
            return null;
        const targetId = Number(input?.target_id);
        if (!Number.isFinite(targetId))
        ...
        if (name === undefined && isEnabled === undefined && isDefault === undefined)
            return null;
        return {
            type: 'update',
            target_id: targetId,
    ...
    normalizeAiTokenSuggestion(input) {
        if (String(input?.type) !== 'update')
            return null;
        const targetId = Number(input?.target_id);
        if (!Number.isFinite(targetId))
        ...
        if (!status && autoSync === undefined && note === undefined)
            return null;
        return {
            type: 'update',
            target_id: targetId,
    ...
    ensureTagByName(name, existingTags) {
    ...
    buildAccountRemark(account) {
        if (account.custom_domain)
            return `自定义域名邮箱 ${account.custom_domain}`;
    ...
    normalizeRuleConfigForTrigger(triggerType, input) {
    ...
    normalizeRuleInput(data, partial = false) {
...
exports.PersonalOSService = PersonalOSService;
//# sourceMappingURL=PersonalOSService.js.map