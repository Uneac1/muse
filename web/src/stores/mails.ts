...
import type { MailMessage, FetchMailsResult, MailboxType } from '../types';
...
interface MailStore {
  ...
  setMailbox: (box: MailboxType) => void;
  ...
  searchCachedMails: (accountId: number, mailbox: string, query: string) => Promise<void>;
...
export const useMailStore = create<MailStore>((set) => ({
  ...
  searchCachedMails: async (accountId, mailbox, query) => {
    set({ loading: true });
    try {
      const data = await mailApi.search({ account_id: accountId, mailbox, query, pageSize: 100 });
      set({ mails: data.list });
    } finally {
      set({ loading: false });
    }
