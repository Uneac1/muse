...
export const ALL_COLUMNS = [
  ...
  { key: 'provider', label: '提供商', defaultVisible: true },
...
interface Props {
  ...
  onViewMail: (account: Account, mailbox: MailboxType) => void;
  loading: boolean;
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-zinc-200 dark:border-zinc-700">
            ...
            {isVisible('provider') && (
              <SortHeader label="提供商" sortKey="provider" currentKey={sortKey} currentDir={sortDir} onSort={handleSort} className="min-w-[90px]" />
            )}
            {isVisible('email') && (
              ...
              {isVisible('provider') && (
                <td className="py-3 px-3">
                  <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
                    ...
                    {account.provider === 'gmail' ? 'Gmail' : account.provider === 'qq' ? 'QQ' : account.provider === 'custom' ? 'Custom' : 'Microsoft'}
                  </span>
                </td>
              )}
              {isVisible('email') && (
