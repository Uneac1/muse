...
import openteams from "./theme/openteams.json" with { type: "json" }
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    const [store, setStore] = createStore({
      ...
      active: (config.theme ?? kv.get("theme", "openteams-cli")) as string,
      ready: false,
    })
    ...
    const values = createMemo(() => {
      return resolveTheme(store.themes[store.active] ?? store.themes["openteams-cli"] ?? store.themes.openteams, store.mode)
    })
